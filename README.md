...
| Props | Houdini 22 | `houdini/diorama_base.hipnc` | A stone slab built for the earlier isometric version: a subdivided box, noise on its sides, faceted, flat shading baked into vertex colours in VEX, exported as a triangle list because Apprentice cannot write FBX. Kept in the repo (with `WorldView` and `MeshText`) but unused since the pivot to the platformer. |
