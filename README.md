...
anyone who falls, and so do the acid pools and the spikes. Vents in the rock
breathe fire on a count. Some ledges crumble a heartbeat after you land on
them. Embers drift along rows and columns and burn what they touch, though
growth puts them out: an ember caught inside a bloom is quenched until the
next rewind. You carry three lanterns per chapter; lose them all and the
dark takes you, then you try again. Ten chapters across four laws, and the
lantern in your hand has a voice.
...
| Tooling | [Nexium](https://github.com/Londopy/nexium) | `build.nx`, `tools/bindgen.nx`, `tools/levels.nx`, `tools/chapters.nx` | `bindgen` reads the `@(export)` procs in the Odin plugin and writes the C# `DllImport` surface. `levels` compiles the `.origin` level files into `levels.json`: it grows each target through the Odin CLI, counts the seed placements that reproduce it, reads the hazard glyphs and the lantern's `say` lines, and reports each chapter's deduction (places to stand inside the outline, and whether the origin sits at its centre). `chapters` writes [docs/chapters.md](docs/chapters.md) from the compiled levels. `build.nx` runs the whole pipeline. |
| Game | C# / Unity 6 (6000.6) | `unity/PointOfOrigin` | The side-scrolling world (painted into one point-filtered texture), the tile platformer physics, the wanderer, embers, acid, spikes, fire spouts, crumbling rock, the lantern's voice (typed-out lines in a bubble, triggered by what you do), keyboard, mouse and gamepad input, HUD and menus. The scene holds only the template camera and light; everything else is built in code at load. |
| Music and sound | C# | `Assets/Scripts/Sfx.cs` | No audio files. A small tracker renders a 16-bit tune at start-up: pulse waves at three duty cycles for lead and counter-melody, a two-operator FM bell for the arpeggios, a triangle bass, soft noise drums, then 8-bit quantisation and a low-pass for the lo-fi warmth. The chapters play it in a different key each; the title plays a slower, sparser take. Every effect (jumps, plants, growth ticks, cracks, thuds, spout whooshes, the acid's bubbling, the embers' hiss, the voice blips, stingers) is synthesised the same way. |
...
column, `~` acid, `^` spikes, `f` a fire spout set into a floor cell, `c`
rock that crumbles, and the tool grows the target for you. `intro` and
`outro` lines carry the story, and `say trigger|line` lines give the lantern
its voice (triggers: wake, seed, plant, grow, found, die):
...
The origin is the puzzle; the road is how you reach it. Each chapter asks
...
cut by their far walls. Two Echoes: two origins two cells apart cancel each
other, leaving a hole where you would look first. Origin: a mesa cut by a
wall and bent by a post. The compiler counts the placements that reproduce
each outline, so every chapter has exactly one answer, and
[docs/chapters.md](docs/chapters.md) lists how many places you could stand
in each outline and whether the stone sits at its centre.
